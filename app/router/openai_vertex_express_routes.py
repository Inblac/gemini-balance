...
from app.log.logger import get_openai_vertex_express_logger
...
logger = get_openai_vertex_express_logger()
...
async def get_openai_chat_service(key_manager: KeyManager = Depends(get_key_manager)):
    """获取OpenAI聊天服务实例"""
    return OpenAIChatService(settings.VERTEX_EXPRESS_BASE_URL, key_manager)
...
async def chat_completion(
    ...
    async with handle_route_errors(logger, operation_name):
        logger.info(f"【VERTEX】Handling chat completion request for model: {request.model}")
        logger.debug(f"Request: \n{request.model_dump_json(indent=2)}")
        logger.info(f"【VERTEX】Using allowed token: {allowed_token}")
        logger.info(f"【VERTEX】Using API key: {current_api_key}")
